...
describe 'Indexes' do
  describe '#add_index' do
    it 'should be built with the :where option' do
      ...
      PgPower::Explorer.index_exists?(:pets, :name, index_options).should be_truthy
    end

    it 'should allow indexes with expressions using functions' do
      ActiveRecord::Migration.add_index(:pets, ["lower(name)", "lower(color)"])

      PgPower::Explorer.index_exists?(:pets, ["lower(name)", "lower(color)"] ).should be_truthy
    end
    ...
    it 'should allow indexes with expressions using functions with multiple arguments' do
      ...
      PgPower::Explorer.index_exists?(:pets, "gin(to_tsvector('english', name))" ).should be_truthy
    end
    ...
    it 'should allow indexes with advanced expressions' do
      ...
      PgPower::Explorer.index_exists?(:pets, ["(color || ' ' || name)"] ).should be_truthy
    end
    ...
    it "should allow partial indexes with expressions" do
      ...
      PgPower::Explorer.index_exists?(:pets, ['upper(name)', 'lower(color)'], opts).should be_truthy
    end
  end
  ...
  describe '#remove_index' do
    it 'should remove indexes with expressions using functions' do
      ...
      PgPower::Explorer.index_exists?(:pets, ["lower(name)", "lower(color)"] ).should be_falsey
    end
    ...
    it 'should remove indexes built with the :where option' do
      ...
      PgPower::Explorer.index_exists?(:pets, :name, index_options).should be_falsey
    end
  end

  describe '#index_exists' do
    it 'should be true for simple options' do
      PgPower::Explorer.index_exists?('pets', :color).should be_truthy
    end
    ...
    it 'should be true for simple options on a schema table' do
      PgPower::Explorer.index_exists?('demography.cities', :country_id).should be_truthy
    end
    ...
    it 'should be true for concurrently created index' do
      ...
      PgPower::Explorer.index_exists?(:users, :email, index_options).should be_truthy
    end
